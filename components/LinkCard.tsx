...
const getHostname = (url: string): string => {
  ...
  try {
    // Ensure the URL has a protocol, otherwise new URL() will fail for strings like "example.com"
    const fullUrl = /^(https?|ftp):\/\//i.test(url) ? url : `https://${url}`;
    return new URL(fullUrl).hostname;
  } catch (error) {
    console.warn(`Could not parse URL to get hostname: "${url}"`, error);
    // Provide a fallback for invalid URLs
